...
function renderChart() {
    var chartWrapper = document.querySelector('#chart-wrap'),
        html = '',
        color = getColors();
    for (var str in chartData) {
        html += '<div title="'+str+":"+chartData[str]+'" style="height:'+chartData[str]+'px; background-color:'+color+'"></div>';
    }
...
function initGraTimeForm() {
    var inputEvent = document.getElementsByTagName('input');
    for (var i = 0, len = inputEvent.length; i < len; i++) {
        inputEvent[i].addEventListener('click', graTimeChange);
    }
}
